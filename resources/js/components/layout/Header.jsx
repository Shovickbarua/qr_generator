...
function Header({
  ...
  const menu = (<List min-width="100%" className=" w-64 bg-white p-4 rounded-lg shadow-lg header-notifications-dropdown " itemLayout="horizontal"
      // dataSource={data}
      dataSource={notes}
      renderItem={(item) => (
        <List.Item>
      ...
      <Row gutter={[24, 0]}>
        <Col span={24} md={8}>
          <Breadcrumb>
            ...
            {pathArr?.map((pathA, index)=>{
            if (pathA) {
              return(<Breadcrumb.Item key={'bred-'+index} style={{ textTransform: "capitalize" }}>
                {pathA}
              </Breadcrumb.Item>)
            }})}
          </Breadcrumb>
        ...
        <Col span={24} md={16} className="header-control">
          <Badge size="small" count={notes?.length}>
          ...
          <Link to="/account" className="btn-sign-in">
            {profile}
            {user?.name ? <span>{user.name}</span> : <span>Sign in</span>}
          </Link>
          <Input
